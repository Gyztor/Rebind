...
namespace Rebind
{
    public class Patch : ResoniteMod
    {
        ...
        [HarmonyPatch(typeof(IndexController))]
        class IndexControllerPatch
        {
            [HarmonyPatch("Bind")]
            [HarmonyPrefix]
            static bool Prefix(InputGroup group, IndexController __instance, Analog2D ___Joystick, Digital ___TriggerClick, Digital ___GripClick, Digital ___ButtonB, Analog ___Trigger, Digital ___ButtonA)
            {
                if (group.Side != __instance.Side)
                {
                    return false;
                ...
                {
                    commonToolInputs.Interact.AddBinding(InputNode.Digital(___TriggerClick), __instance);
                    commonToolInputs.Secondary.AddBinding(InputNode.Digital(___ButtonA), __instance);
                    commonToolInputs.Grab.AddBinding(InputNode.Digital(___GripClick), __instance);
                    commonToolInputs.Menu.AddBinding(InputNode.Digital(___ButtonB), __instance);
                    commonToolInputs.Strength.AddBinding(InputNode.Analog(___Trigger), __instance);
                    commonToolInputs.Axis.AddBinding(InputNode.Analog2D(___Joystick), __instance);
                }
            ...
            [HarmonyPrefix]
            [HarmonyPatch("BindNodeActions")]
            static bool PrefixBindActions(IndexController __instance, Digital ___ButtonA, IInputNode node, string name = null)
            {
                IDualAxisInputNode dualAxisInputNode = node as IDualAxisInputNode;
                if (dualAxisInputNode == null)
                {
                    ...
                    if (anyInput == null)
                    {
                        ...
                        else
                        {
                            switch (name)
                            {
                                case "Jump":
                                case "Align":
                                    leftRightSelector.SetNode(__instance.Side, InputNode.Digital(__instance.Side, ___ButtonA.Name));
                                    break;
                                case "AnchorAction":
                                    leftRightSelector.SetNode(__instance.Side, InputNode.Digital(__instance.Side, ___ButtonA.Name));
                                    break;
                    ...
                    {
                        anyInput.Inputs.Add(InputNode.Digital(__instance.Side, ___ButtonA.Name));
                    }
                }
                else
                {
